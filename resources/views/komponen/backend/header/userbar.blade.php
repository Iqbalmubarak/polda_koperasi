...
<div class="kt-header__topbar-item kt-header__topbar-item--user">
  <div class="kt-header__topbar-wrapper" data-toggle="dropdown" data-offset="0px,0px">
    <div class="kt-header__topbar-user">
      ...
      <span class="kt-header__topbar-username kt-hidden-mobile">{{Sentinel::getUser()->first_name}}</span>
      @if(Sentinel::getUser()->avatar)
        <img class="" alt="Pic" src="{{url('img/profile-pict/'.Sentinel::getUser()->avatar)}}" />
      @else
        <span class="kt-badge kt-badge--username kt-badge--unified-success kt-badge--lg kt-badge--rounded kt-badge--bold">{{Helper::awalan(Sentinel::getUser()->first_name)}}</span>
      @endif
    ...
    <div class="kt-user-card kt-user-card--skin-dark kt-notification-item-padding-x" style="background-image: url({{asset('theme/media/misc/bg-1.jpg')}})">
      <div class="kt-user-card__avatar">
        @if(Sentinel::getUser()->avatar)
          <img class="" alt="Pic" src="{{url('img/profile-pict/'.Sentinel::getUser()->avatar)}}" />
        @else
          <span class="kt-badge kt-badge--lg kt-badge--rounded kt-badge--bold kt-font-success">{{Helper::awalan(Sentinel::getUser()->first_name)}}</span>
        @endif
        <!--use below badge element instead the user avatar to display username's first letter(remove kt-hidden class to display it) -->
      </div>
