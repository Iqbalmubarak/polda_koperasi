...
@section('title') Transaksi @stop

@section('subheader-name') Transaksi @stop
...
<script type="text/javascript">
  ...
  jQuery(document).ready(function() {
    var table = $('#tbl_inreq').DataTable({
      ...
      // serverSide: true,
      ajax: {
          url: "{{url('api/transaksi')}}?data=all&status={{$status}}",
          dataSrc: ''
      },
      columns: [
        {data: 'id'},
      ...
      columnDefs: [
        {
          ...
          render: function(data, type, full, meta) {
            var base = "{{url('/')}}";
            var hasil = ``+
              @if(Sentinel::getUser()->hasAccess(['transaksi.show']))
                    ` <a class="btn btn-success btn-sm" href="`+base+'/transaksi/'+data+`{{isset($_GET['status']) ? '?status='.$_GET['status'] : ''}}">@lang('global.app_detail')</a>`+
              @endif
                    ``;
            if(full.status == "Diproses"){
              hasil = hasil +
              @if(Sentinel::getUser()->hasAccess(['transaksi.destroy']))
