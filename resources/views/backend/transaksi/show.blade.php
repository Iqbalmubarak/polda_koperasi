...
@section('title') @lang('global.app_detail') {{$name}} @stop

@section('subheader-name') @lang('global.app_detail') {{$name}} @stop

@section('subheader-link')
  <span class="kt-subheader__breadcrumbs-separator"></span>
  <a href="javascript:void(0)" class="kt-subheader__breadcrumbs-link">
    @lang('global.app_detail') {{$name}}
  </a>
@stop

@section('subheader-btn')
  @if(Sentinel::getUser()->hasAccess(['transaksi.index']))
  <a href="{{route('transaksi.index')}}{{Sentinel::getUser()->hasAccess('transaksi.all-data') ? '?status='.$status : ''}}" class="btn btn-default btn-bold">
    @lang('global.app_back_to_list')
  </a>
  @endif

    @if(Sentinel::getUser()->hasAccess(['transaksi.send']) && $data->status==1)
  <a href="javascript:void(0)" onclick="send({{$data->id}})" class="btn btn-success btn-bold">
    Kirim Barang
  </a>
  @endif
    ...
    <table class="table table-striped table-checkable" id="tbl_inreq">
      <thead>
        <tr>
          ...
          <th>: {{date_format($data->created_at, "d-m-Y")}}</th>
          <th>@lang('inventorisRequest.show.status')</th>
          <th>: @if($data->status==1) Diproses @elseif($data->status==2) Diterima @else Dibatalkan @endif</th>
          <th></th>
          <th></th>
        </tr>
        <tr>
          <th>Nama Pelanggan</th>
          <th>: {{$data->users->first_name}} {{$data->users->last_name}}</th>
          <th>Total Harga</th>
          <?php 
            $price = 0;
            foreach($data->details as $detail){
              $price = $price + (($detail->total) * ($detail->items->modal + $detail->items->price));
            }
          ?>
          <th>: {{$price}}</th>
          <th></th>
      ...
      <tbody>
        @foreach($data->details as $detail)
          <tr>
            ...
            <td>{{$detail->items->units->name}}</td>
            <td>{{$detail->total}}</td>
            <td>{{($detail->total) * ($detail->items->modal + $detail->items->price)}}</td>
          </tr>
        @endforeach
      </tbody>
    </table>
        ...
        <div class="modal-body">
          <div class="col-md-12">
            <div class="kt-form__group--inline">
              ...
              <div class="kt-form__control">
                {!! Form::select('brands', $brands, null, ['class' => 'form-control kt-select2 myselect2', 'id'=>'s_barang_create']) !!}
                @error('brands')
          ...
          <div class="col-md-12">
            <div class="kt-form__group--inline">
              ...
              <div class="kt-form__control">
                {!! Form::select('units', $units, null, ['class' => 'form-control kt-select2 myselect2', 'id'=>'s_satuan_create']) !!}
                @error('units')
                  <div class="form-text text-danger">{{$message}}</div>
                @enderror
              </div>
        ...
        <div class="modal-body">
          <div class="col-md-12">
            <div class="kt-form__group--inline">
              ...
              <div class="kt-form__control">
                {!! Form::select('brands', $brands, null, ['class' => 'form-control kt-select2 myselect2', 'id'=>'s_barang_update', Sentinel::getUser()->hasAccess('inventoris.all-data') ? 'disabled' : '']) !!}
                @error('brands')
                  <div class="form-text text-danger">{{$message}}</div>
                @enderror
              </div>
          ...
          <div class="col-md-12">
            <div class="kt-form__group--inline">
              ...
              <div class="kt-form__control">
                {!! Form::select('units', $units, null, ['class' => 'form-control kt-select2 myselect2', 'id'=>'s_satuan_update', Sentinel::getUser()->hasAccess('inventoris.all-data') ? 'disabled' : '']) !!}
                @error('units')
                  <div class="form-text text-danger">{{$message}}</div>
                @enderror
              </div>
...
@section('tjs')
  ...
  <script type="text/javascript">
    ...
    function send(id) {
      swal.fire({
          title: "Kirimkan Barang Ini?",
          text: "Pemesanan Yang  Telah Dikonfirmasi Akan Dikirim Kepada Pelanggan",
          type: 'warning',
          showCancelButton: true,
          ...
          if (result.value) {
              ...
              $('#acc_f').attr('action', "{{route('transaksi.index')}}/"+id);
              $('#acc_f').submit();
          } else if (result.dismiss === 'cancel') {
              swal.fire(
                  "@lang('global.app_deleted_cancel_massage_1')",
