...
<a href="#" class="btn btn-label-brand btn-bold">Rp. {{$harga}},00</a>
<a href="{{route('orders.confirm', $id)}}" class="btn btn-label-brand btn-bold">Check Out</a>
@stop
...
<div class="kt-portlet__head-label">
    ...
    @lang('global.app_list') Keranjang
    </h3>
</div>
...
swal.fire({
    ...
    if (result.value) {
        ...
        $('#form-delete').attr('action', "{{route('orders.index')}}/"+id);
        $('#form-delete').submit();
...
if($('#edit').is(":visible") && id==id_awal){
    ...
    $('#edit').show('500');
    $('#e_name').val(name);
    $('#form-update').attr('action', "{{route('orders.index')}}/"+id);
}
