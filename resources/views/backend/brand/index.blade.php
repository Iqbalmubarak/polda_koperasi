...
@section('subheader-btn')
  @if (Sentinel::getUser()->hasAccess(['brand.add']))
...
@endif
@if (Sentinel::getUser()->hasAccess(['brand.updt']))
  ...
  <div class="kt-portlet__head kt-portlet__head--lg">
    <div class="kt-portlet__head-label">
      ...
      <h3 class="kt-portlet__head-title">
        @lang('global.app_list') Merek Barang
      </h3>
    </div>
...
{!! Form::open(['method'=>'DELETE', 'route' => ['brand.destroy', 0], 'style' => 'display:inline','id'=>'form-delete']) !!}
{!! Form::close() !!}
@stop
...
<script type="text/javascript">
      ...
      columnDefs: [
        {
          ...
          render: function(data, type, full, meta) {
            ...
            return ``+
              ...
              @endif
              @if(Sentinel::getUser()->hasAccess(['brand.destroy']))
  ...
  function delete_satbar(id) {
    swal.fire({
        ...
        if (result.value) {
            ...
            $('#form-delete').attr('action', "{{route('brand.index')}}/"+id);
            $('#form-delete').submit();
  ...
  function edit(id, name) {
    if($('#edit').is(":visible") && id==id_awal){
      ...
      $('#edit').show('500');
      $('#e_name').val(name);
      $('#form-update').attr('action', "{{route('brand.index')}}/"+id);
    }
