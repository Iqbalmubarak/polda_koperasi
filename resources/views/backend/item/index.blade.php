...
@if (Sentinel::getUser()->hasAccess(['item.create']))
  <a href="{{route('item.create')}}?status={{$status_name}}" class="btn btn-label-brand btn-bold">@lang('global.app_create') {{$name}}</a>
@endif
@stop
...
<div class="kt-portlet__body">
  ...
  <form class="kt-form kt-form--fit kt-margin-b-20">
    <div class="row kt-margin-b-20">
      ...
      <div class="col-lg-3 kt-margin-b-10-tablet-and-mobile">
        <label>Merek:</label>
        <select class="form-control kt-input myselect2" data-col-index="4">
          <option value="">Select</option>
        </select>
      </div>
...
{!! Form::open(['method'=>'DELETE', 'route' => ['item.destroy', 0], 'style' => 'display:inline','id'=>'form-delete']) !!}
{!! Form::close() !!}
    ...
    {{ Form::open(array('method'=>'PATCH', 'url' => '#', 'files' => true, 'id'=>'form_edit')) }}
      <div class="modal-body">
        ...
        <div class="col-md-12">
          <div class="kt-form__group--inline">
            <div class="kt-form__label">
              <label class="kt-label m-label--single">Nama Barang:</label>
            </div>
            <div class="kt-form__control">
              {!! Form::text('name',null, ['class' => 'form-control','required'=>'required', 'id'=>'s_name_update']) !!}
                @error('name')
        ...
        <div class="col-md-12">
          <div class="kt-form__group--inline">
            <div class="kt-form__label">
              <label class="kt-label m-label--single">Satuan:</label>
            </div>
            <div class="kt-form__control">
              {!! Form::select('units', $units, null, ['class' => 'form-control kt-select2 myselect2', 'id'=>'s_unit_update']) !!}
              @error('units')
                <div class="form-text text-danger">{{$message}}</div>
              @enderror
            </div>
        ...
        <div class="col-md-12">
          <div class="kt-form__group--inline">
            <div class="kt-form__label">
              <label class="kt-label m-label--single">Jenis:</label>
            </div>
            <div class="kt-form__control">
              {!! Form::select('type', $type, null, ['class' => 'form-control kt-select2 myselect2', 'id'=>'s_type_update']) !!}
              @error('type')
                <div class="form-text text-danger">{{$message}}</div>
              @enderror
            </div>
        ...
        <div class="col-md-12">
          <div class="kt-form__group--inline">
            <div class="kt-form__label">
              <label class="kt-label m-label--single">Merek:</label>
            </div>
            <div class="kt-form__control">
              {!! Form::select('brands', $brands, null, ['class' => 'form-control kt-select2 myselect2', 'id'=>'s_brands_update']) !!}
              @error('brands')
                <div class="form-text text-danger">{{$message}}</div>
              @enderror
            </div>
        ...
        <div class="col-md-12">
          <div class="kt-form__group--inline">
            <div class="kt-form__label">
              <label class="kt-label m-label--single">Profit:</label>
            </div>
            <div class="kt-form__control">
        ...
        render: function(data, type, full, meta) {
          ...
          return ``+
            @if(Sentinel::getUser()->hasAccess(['item.updt']))
                  ` <a class="btn btn-success btn-sm" href="javascript::void(0)" onclick="edit_inventoris(`+data+`,`+full.unit_id+`,`+full.type_id+`)">@lang('global.app_edit')</a>`+
            @endif
            @if(Sentinel::getUser()->hasAccess(['item.show']))
                  ` <a class="btn btn-success btn-sm" href="`+base+'/item/'+data+`">@lang('global.app_detail')</a>`+
            @endif
...
function delete_inventoris(id) {
  swal.fire({
      ...
      if (result.value) {
          ...
          $('#form-delete').attr('action', "{{route('item.index')}}/"+id);
          $('#form-delete').submit();
...
function edit_inventoris(id, units, type) {
  $("#s_unit_update").val(units).trigger('change');
  $("#s_type_update").val(type).trigger('change');
  $('#f_modal_edit').modal('show');
  var base = "{{url('/')}}";
  $('#form_edit').attr('action', base+'/item/'+id);
}
