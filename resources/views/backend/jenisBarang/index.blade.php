...
@section('subheader-btn')
  @if (Sentinel::getUser()->hasAccess(['jenis.add']))
...
@if (Sentinel::getUser()->hasAccess(['jenis.add']))
  @include('backend.jenisBarang.create')
@endif
@if (Sentinel::getUser()->hasAccess(['jenis.updt']))
  @include('backend.jenisBarang.edit')
@endif
...
<div class="kt-portlet kt-portlet--mobile">
  ...
  <div class="kt-portlet__body">
    <!--begin: Datatable -->
    <table class="table table-striped- table-bordered table-hover table-checkable" id="tbl_jenis">
      <thead>
        <tr>
          <th>No</th>
          <th>Jenis Barang</th>
...
{!! Form::open(['method'=>'DELETE', 'route' => ['jenis.destroy', 0], 'style' => 'display:inline','id'=>'form-delete']) !!}
{!! Form::close() !!}
@stop
...
<script type="text/javascript">
  ...
  jQuery(document).ready(function() {
    var table = $('#tbl_jenis').DataTable({
      ...
      // serverSide: true,
      ajax: {
          url: "{{url('api/jenis-barang')}}?data=all",
          dataSrc: ''
      },
      columns: [
        {data: 'id'},
      ...
      columnDefs: [
        {
          ...
          render: function(data, type, full, meta) {
            ...
            return ``+
              @if(Sentinel::getUser()->hasAccess(['jenis.updt']))
                   ` <a class="btn btn-success btn-sm" href="javascript::void(0)" onclick="edit(`+data+`,'`+full.nama_jenis+`')">@lang('global.app_edit') {{$name}}</a>`+
              @endif
              @if(Sentinel::getUser()->hasAccess(['jenis.destroy']))
                   ` <a class="btn btn-danger btn-sm" onclick="delete_jenis(`+data+`)" href="javascript::void(0)">@lang('global.app_delete') {{$name}}</a>`+
              @endif
                   ``;
          },
        },
  ...
  function delete_jenis(id) {
    swal.fire({
        ...
        if (result.value) {
            ...
            $('#form-delete').attr('action', "{{route('jenis.index')}}/"+id);
            $('#form-delete').submit();
  ...
  function edit(id, name) {
    if($('#edit').is(":visible") && id==id_awal){
      ...
      $('#edit').show('500');
      $('#e_name').val(name);
      $('#form-update').attr('action', "{{route('jenis.index')}}/"+id);
    }
