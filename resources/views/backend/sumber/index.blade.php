...
@section('subheader-btn')
  @if (Sentinel::getUser()->hasAccess(['sumber.add']))
...
@if (Sentinel::getUser()->hasAccess(['sumber.add']))
  @include('backend.sumber.create')
@endif
@if (Sentinel::getUser()->hasAccess(['sumber.updt']))
  @include('backend.sumber.edit')
@endif
  ...
  <div class="kt-portlet__head kt-portlet__head--lg">
    <div class="kt-portlet__head-label">
      ...
      <h3 class="kt-portlet__head-title">
        @lang('global.app_list') @lang('sumber.r_title')
      </h3>
    </div>
      ...
      columnDefs: [
        {
          ...
          render: function(data, type, full, meta) {
            ...
            return ``+
              ...
              @endif
              @if(Sentinel::getUser()->hasAccess(['sumber.destroy']))
                   ` <a class="btn btn-danger btn-sm" onclick="delete_role(`+data+`)" href="javascript::void(0)">@lang('global.app_delete') {{$name}}</a>`+
              @endif
                   ``;
          },
        },
  ...
  function delete_role(id) {
    swal.fire({
        ...
        if (result.value) {
            ...
            $('#form-delete').attr('action', "{{route('sumber.index')}}/"+id);
            $('#form-delete').submit();
