  ...
  <div class="kt-portlet__head kt-portlet__head--lg">
    <div class="kt-portlet__head-label">
      ...
      <h3 class="kt-portlet__head-title">
        @lang('global.app_list') @lang('satuanKerja.title')
      </h3>
    </div>
    ...
    var table = $('#tbl_satker').DataTable({
      ...
      columnDefs: [
        ...
        },
        {
          ...
          render: function(data, type, full, meta) {
            var base = "{{url('/')}}";
            return `<a class="btn btn-success btn-sm" href="javascript:void(0)" onclick="show('`+full.name+`','`+full.path_logo+`');">@lang('global.app_view') {{$name}}</a>`+
              @if(Sentinel::getUser()->hasAccess(['satker.updt']))
                   ` <a class="btn btn-success btn-sm" href="javascript::void(0)" onclick="edit(`+data+`,'`+full.status+`','`+full.name+`')">@lang('global.app_edit') {{$name}}</a>`+
              @endif
              @if(Sentinel::getUser()->hasAccess(['satker.destroy']))
                   ` <a class="btn btn-danger btn-sm" onclick="delete_satker(`+data+`)" href="javascript::void(0)">@lang('global.app_delete') {{$name}}</a>`+
              @endif
                   ``;
          },
        },
