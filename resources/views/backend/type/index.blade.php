...
@section('subheader-btn')
  @if (Sentinel::getUser()->hasAccess(['type.add']))
...
@endif
@if (Sentinel::getUser()->hasAccess(['type.updt']))
...
{!! Form::open(['method'=>'DELETE', 'route' => ['type.destroy', 0], 'style' => 'display:inline','id'=>'form-delete']) !!}
{!! Form::close() !!}
@stop
...
<script type="text/javascript">
      ...
      columnDefs: [
        {
          ...
          render: function(data, type, full, meta) {
            ...
            return ``+
              ...
              @endif
              @if(Sentinel::getUser()->hasAccess(['type.destroy']))
  ...
  function delete_satbar(id) {
    swal.fire({
        ...
        if (result.value) {
            ...
            $('#form-delete').attr('action', "{{route('type.index')}}/"+id);
            $('#form-delete').submit();
  ...
  function edit(id, name) {
    if($('#edit').is(":visible") && id==id_awal){
      ...
      $('#edit').show('500');
      $('#e_name').val(name);
      $('#form-update').attr('action', "{{route('type.index')}}/"+id);
    }
