...
@if (Sentinel::getUser()->hasAccess(['supplier.add']))
  <a href="javascript::void(0)" onclick="$('#create').toggle(500);$('#edit').hide(500);" class="btn btn-label-brand btn-bold">Tambah Data Supplier</a>
@endif
@stop
...
@endif
@if (Sentinel::getUser()->hasAccess(['supplier.updt']))
...
jQuery(document).ready(function() {
  var table = $('#tbl_supplier').DataTable({
    ...
    // serverSide: true,
    ajax: {
        url: "{{url('api/supplier-barang')}}?data=all",
        dataSrc: ''
    },
    columns: [
      {data: 'id'},
    ...
    columnDefs: [
      {
        ...
        render: function(data, type, full, meta) {
          ...
          return ``+
            @if(Sentinel::getUser()->hasAccess(['supplier.updt']))
                  ` <a class="btn btn-success btn-sm" href="javascript::void(0)" onclick="edit(`+data+`,'`+full.name+`','`+full.alamat+`')">@lang('global.app_edit') {{$name}}</a>`+
            @endif
            @if(Sentinel::getUser()->hasAccess(['supplier.destroy']))
                  ` <a class="btn btn-danger btn-sm" onclick="delete_supplier(`+data+`)" href="javascript::void(0)">@lang('global.app_delete') {{$name}}</a>`+
            @endif
                  ``;
        },
      },
