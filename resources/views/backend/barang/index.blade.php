...
@section('subheader-btn')
  @if (Sentinel::getUser()->hasAccess(['barang.add']))
...
@endif
@if (Sentinel::getUser()->hasAccess(['barang.updt']))
...
<div class="kt-portlet kt-portlet--mobile">
  <div class="kt-portlet__head kt-portlet__head--lg">
    <div class="kt-portlet__head-label">
      ...
      <h3 class="kt-portlet__head-title">
        @lang('global.app_list') @lang('barang.title')
      </h3>
    </div>
  ...
  <div class="kt-portlet__body">
    <!--begin: Datatable -->
    <table class="table table-striped- table-bordered table-hover table-checkable" id="tbl_barang">
      <thead>
        <tr>
          <th>No</th>
          <th>@lang('barang.field.name')</th>
...
{!! Form::open(['method'=>'DELETE', 'route' => ['barang.destroy', 0], 'style' => 'display:inline','id'=>'form-delete']) !!}
{!! Form::close() !!}
@stop
...
<script type="text/javascript">
      ...
      columnDefs: [
        {
          ...
          render: function(data, type, full, meta) {
            ...
            return ``+
              @if(Sentinel::getUser()->hasAccess(['barang.updt']))
                   ` <a class="btn btn-success btn-sm" href="javascript::void(0)" onclick="edit(`+data+`,'`+full.nama_barang+`')">@lang('global.app_edit') {{$name}}</a>`+
              @endif
              @if(Sentinel::getUser()->hasAccess(['barang.destroy']))
                   ` <a class="btn btn-danger btn-sm" onclick="delete_barang(`+data+`)" href="javascript::void(0)">@lang('global.app_delete') {{$name}}</a>`+
              @endif
                   ``;
          },
        },
  ...
  function delete_barang(id) {
    swal.fire({
        ...
        if (result.value) {
            ...
            $('#form-delete').attr('action', "{{route('barang.index')}}/"+id);
            $('#form-delete').submit();
  ...
  function edit(id, name) {
    if($('#edit').is(":visible") && id==id_awal){
      ...
      $('#edit').show('500');
      $('#e_name').val(name);
      $('#form-update').attr('action', "{{route('barang.index')}}/"+id);
    }
